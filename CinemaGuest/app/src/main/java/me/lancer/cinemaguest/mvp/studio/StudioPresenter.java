package me.lancer.cinemaguest.mvp.studio;

import java.util.List;

import me.lancer.cinemaguest.mvp.base.IBasePresenter;

/**
 * Created by HuangFangzhi on 2017/3/13.
 */

public class StudioPresenter implements IBasePresenter<IStudioView>, IStudioPresenter {

    private IStudioView view;
    private StudioModel model;

    public StudioPresenter(IStudioView view) {
        attachView(view);
        model = new StudioModel(this);
    }

    @Override
    public void attachView(IStudioView view) {
        this.view = view;
    }

    @Override
    public void detachView() {
        this.view = null;
    }

    public void add(String name, String introduction, String status, String rows, String cols, String session) {
        if (view != null) {
            view.showLoad();
            model.add(name, introduction, status, rows, cols, session);
        }
    }

    @Override
    public void addSuccess(String result) {
        if (view != null) {
            view.showAdd(result);
            view.hideLoad();
        }
    }

    @Override
    public void addFailure(String result) {
        if (result != null && result.length() > 0 && view != null) {
            view.showMsg(result);
            view.hideLoad();
        }
    }

    public void fetch(String id, String name, String introduction, String status, String rows, String cols, String session) {
        if (view != null) {
            view.showLoad();
            model.fetch(id, name, introduction, status, rows, cols, session);
        }
    }

    @Override
    public void fetchSuccess(List<StudioBean> list) {
        if (view != null) {
            view.showFetch(list);
            view.hideLoad();
        }
    }

    @Override
    public void fetchFailure(String result) {
        if (result != null && result.length() > 0 && view != null) {
            view.showMsg(result);
            view.hideLoad();
        }
    }

    public void modify(String id, String name, String introduction, String status, String rows, String cols, String session) {
        if (view != null) {
            view.showLoad();
            model.modify(id, name, introduction, status, rows, cols, session);
        }
    }

    @Override
    public void modifySuccess(String result) {
        if (view != null) {
            view.showModify(result);
            view.hideLoad();
        }
    }

    @Override
    public void modifyFailure(String result) {
        if (result != null && result.length() > 0 && view != null) {
            view.showMsg(result);
            view.hideLoad();
        }
    }

    public void delete(String id, String session) {
        if (view != null) {
            view.showLoad();
            model.delete(id, session);
        }
    }

    @Override
    public void deleteSuccess(String result) {
        if (view != null) {
            view.showDelete(result);
            view.hideLoad();
        }
    }

    @Override
    public void deleteFailure(String result) {
        if (result != null && result.length() > 0 && view != null) {
            view.showMsg(result);
            view.hideLoad();
        }
    }
}
